(function($) {
	...
	$(function() {
		...
		function addFile(file) {
			//alert("fullPath:"+file);
			//console.log(file);
			var $li = $('<li id="' + file.id+1 + '">' + '<p class="title">'
			...
			if (state === 'ready') {
				...
				if (stats.uploadFailNum) {
					text = '已成功上传'
							...
							+ '张照片至XX相册，'
							+ stats.uploadFailNum
							+ '张照片上传失败，<a class="retry" href="javascript:void(0)">重新上传</a>失败图片或<a class="ignore" href="#">忽略</a>'
				}
		...
		uploader.onUploadProgress = function(file, percentage) {
			var $li = $('#' + file.id +1), $percent = $li.find('.progress span');
