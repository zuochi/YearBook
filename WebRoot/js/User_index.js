...
//上传图片
							...
							$("#photosUL2").append(
								"<li value='"+json[i].id+"'>"+
									"<div id='comment"+json[i].id+"' class='text2'>"+
										"<textarea rows='3' cols='50' id='reply"+json[i].id+"' onfocus='friendNameTipsInit("+json[i].id+")' onkeyup='getAtName(this.value.charAt(value.length-1),"+json[i].id+")'></textarea>"+
		...
		//局部加载(已经写入cbpGridGallery.js)
		/* document.onscroll = function(){
			//alert(document.body.scrollTop + ":" + document.body.clientHeight + ":" +document.body.scrollHeight);
		    if(document.body.scrollTop+document.body.clientHeight>=(document.body.scrollHeight)){//630=最后一个减去前面2个
		    	toPageP+=1;
		    	getPhotosByPerPage("false");
		    	//alert(document.body.scrollTop + ":" + document.body.clientHeight + ":" +document.body.scrollHeight);
		    }
		}; */
